function [F1, G] = P2LMGA65(xsA, xeA, Pn1_wA, Pn2_wA, xsB, xeB, Pn1_wB, Pn2_wB, cH, A, C_truth, Lheight)
...
ProblemParameters = SetProblemParameters65(GAParameters);

%-----------------------------------------
%   Initialization of the first population Po
%-----------------------------------------
Po = InitialisationNSGA265(individuVide, ProblemParameters,GAParameters);
G = 1;

%-----------------------------------------
%   Po (P) evaluation
%-----------------------------------------
P = EvaluationNSGA265(Po,GAParameters, ProblemParameters);
...
%-----------------------------------------
[P, F] = NonDominatedSorting65(P);

%-----------------------------------------
%   Crowding Distance
%-----------------------------------------
P = CrowdingDistance65(P,F);

%-----------------------------------------
%  Sorting by row then by distance
%-----------------------------------------
[P, ~] = SortPopulation65(P);
...
while (G < GAParameters.Gmax)
    ...
    %-----------------------------------------
    MP = SelectionTournoi65(individuVide, P);

    %-----------------------------------------
    %   Crossover (simulated binary crossover)
    %-----------------------------------------
    Enfants = SimulatedBinaryCrossover65(individuVide, GAParameters, ProblemParameters, MP);

    %-----------------------------------------
    %   Mutation (polynomial mutation)
    %-----------------------------------------
    Mutants = PolynomialMutation65(GAParameters, ProblemParameters, Enfants);
  
%     SelCh=SA_Chrom(Mutants,GAParameters,ProblemParameters);
%     
%     Mutants = [Mutants;SelCh];
    
    Mutants = EvaluationNSGA265(Mutants,GAParameters, ProblemParameters);
    ...
    %-----------------------------------------
    [R, F] = NonDominatedSorting65(R);
    R = CrowdingDistance65(R,F);
    
    %-----------------------------------------
    %   Sorting by row then by distance
    %-----------------------------------------
    [Q, ~] = SortPopulation65(R);
