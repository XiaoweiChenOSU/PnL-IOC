...
function Pop = InitialisationNSGA271(individuVide, ProblemParameters,GAParameters)
...
%     N = 5;
    ...
    for i = 1:N
        ...
        Pop(i).Val(2) = Pop(i).Val(1)+ ceil(Lwidth/5)*(rand(1,1)-0.5);
        if Pop(i).Val(2) > s2U
            Pop(i).Val(2) = s2U;
        ...
        Pop(i).Val(4) = Pop(i).Val(3) + ceil(Lwidth/5)*(rand(1,1)-0.5);
        if  Pop(i).Val(4) < s4L
            Pop(i).Val(4) = s4L;
