...
function Enfants = SimulatedBinaryCrossover71(individuVide, GAParameters, ProblemParameters, MP)
...
for i = 1:2:N
    ...
    if(rand < pc)
        ...
        for k = 1:2
            Enfants(j,k).Val(1:4) = alpha*(P1.Val(1:4)+P2.Val(1:4)) + alpha*Bi*(P1.Val(1:4)-P2.Val(1:4));
        end
        j = j + 1;
    else
...
for i = 1:size(Enfants)
    ...
    if  Enfants(i).Val(4) < s4L
        Enfants(i).Val(4) = s4L;
    elseif Enfants(i).Val(4) > s4U
        Enfants(i).Val(4) = s4U;                    
    end
