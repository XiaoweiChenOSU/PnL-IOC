function [F1, G] = P2LMGA61(xsA, xeA, Pn1_wA, Pn2_wA, xsB, xeB, Pn1_wB, Pn2_wB, A, C_truth, Lheight)
...
GAParameters.Lheight = Lheight;
GAParameters.C_truth = C_truth; %temp

ProblemParameters = SetProblemParameters61(GAParameters);

%-----------------------------------------
%   Initialization of the first population Po
%-----------------------------------------
Po = InitialisationNSGA261(individuVide, ProblemParameters,GAParameters);
G = 1;

%-----------------------------------------
%   Po (P) evaluation
%-----------------------------------------
P = EvaluationNSGA261(Po,GAParameters,ProblemParameters);
...
%-----------------------------------------
[P, F] = NonDominatedSorting61(P);

%-----------------------------------------
%   Crowding Distance
%-----------------------------------------
P = CrowdingDistance61(P,F);

%-----------------------------------------
%  Sorting by row then by distance
%-----------------------------------------
[P, ~] = SortPopulation61(P);
...
while (G < GAParameters.Gmax)
    ...
    %-----------------------------------------
    MP = SelectionTournoi61(individuVide, P);

    %-----------------------------------------
    %   Crossover (simulated binary crossover)
    %-----------------------------------------
    Enfants = SimulatedBinaryCrossover61(individuVide, GAParameters, ProblemParameters, MP);

    %-----------------------------------------
    %   Mutation (polynomial mutation)
    %-----------------------------------------
    Mutants = PolynomialMutation61(GAParameters, ProblemParameters, Enfants);
  
%     SelCh=SA_Chrom(Mutants,GAParameters,ProblemParameters);
%     
%     Mutants = [Mutants;SelCh];
    
    Mutants = EvaluationNSGA261(Mutants,GAParameters,ProblemParameters);
    ...
    %-----------------------------------------
    [R, F] = NonDominatedSorting61(R);
    R = CrowdingDistance61(R,F);
    
    %-----------------------------------------
    %   Sorting by row then by distance
    %-----------------------------------------
    [Q, ~] = SortPopulation61(R);
