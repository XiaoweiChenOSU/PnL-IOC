function [R_wc,t_wc, aPointsN, Oaerrs] = ASPnLATest(p1,p2,W1,W2,cH,R,p2d,IA)
% ,minErr 
...
%         Rr = Roptimzation(p1(1:2,:), p2(1:2,:), IA, Rr, d, 10);
        
        [Rt,aPoints] = CalculateTbyR(p1, p2, W1, W2, cH, Rr);
%         [Rt,aPoints] = Copy_of_CalculateTbyR(p1, p2, W1, W2, cH, Rr);
...
%         tempW2(1,1) = P2_w_real(1,1);
%         tempW2(2,2) = P2_w_real(2,2);
%         tempW2(3,3) = P2_w_real(3,3);
        Pn1_w1 = [W1 tempW2];
        ...
        for i = 1:5
            ...
            [EEcH,GGcH,CCcH] = refineEquationForTest(pn1, pn2,Pn1_w1,Pn2_w1,cH);
...
%             [RAt,aPointsCh] = Copy_of_CalculateTbyR(p1, p2, W1, W2, cH, RrA);
...
%             Pn2_w1 = [tempW2 tempW2(:,2:3) tempW2(:,1)]; 
            ...
            solutionO = [Cayley(RrA);aPointsCh]; 
            [solution, obj_cur] = RefineGaussNewtonRandApT3(solutionO,p,W,R,cH,IA,p2d);
            if size(solution) ~= 12
...
%         [RrA, RAt] = Hidden_PnL(pn1(1:2,:), pn2(1:2,:), Pn1_w1, Pn2_w1);
