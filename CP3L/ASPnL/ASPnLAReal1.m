function [R, T, err, aPointsN] = ASPnLAReal1(p1, p2, P1_w, P2_w,IA)
...
%     tR = Roptimzation(p1(1:2,:), p2(1:2,:), IA, tR, d, 20);
    ...
    aPoints = CalculateAbyR1(p1, p2, P1_w, P2_w, tR);
    
    
    
    p = [p1(:,1) p2];
    W = [P1_w(:,1) P2_w];
    [EE,GG,CC] = RefineEquA1(p, W);
    solutionO = [Cayley(tR);aPoints]; 
    [solution, obj_cur] = RefineGaussNewtonRandApT1(solutionO,EE,GG, CC, p1, p2, P1_w, P2_w);
    if size(solution) ~= 13
...
%     
%     pn1 = [p1 p2(:,2:3) p2(:,2:2:5)];
%     pn2 = [p2 p2(:,4:5) p2(:,3:2:5)];
