function [R, T, err] = ASPnLA0(p1, p2, P1_w, P2_w,IA)
...
%	 T: estimated translation
    ...
    d=P2_w-P1_w;
    d = xnorm(d);
    tR = Roptimzation(p1(1:2,:), p2(1:2,:), IA, tR, d, 20);
    
    %calculate auxiliary points
    aPoints = CalculateAbyR0(p1, p2, P1_w, P2_w, tR);
    
%     %Refine auxiliary points

    solutionO = [Cayley(tR);aPoints]; 
%     [EEA,GGA,CCA] = Copy_2_of_RefineEquA(p1, p2, P1_w, P2_w);
     

    

%     [EEA,GGA,CCA] = RefineEquA(p1, p2, P1_w, P2_w);
    [EEA,GGA,CCA] = Copy_of_RefineEquA0(p2,P2_w);
%   
    [solution, obj_cur] = Copy_of_RefineGaussNewtonRandApT0(solutionO,EEA,GGA);

%     [Rt,aPoints] = RefineEquACopy(p1, p2, P1_w, P2_w, tR);
    if size(solution) ~= 13
...
%     pn1 = [p1 p2(:,5:8) p2(:,5:6) p1(:,1:4) p1(:,1:4)];
%     pn2 = [p2 p2(:,6:8) p2(:,5) p2(:,7:8) p2(:,6) p2(:,5) p2(:,8) p2(:,7) p2(:,8) p2(:,7) p2(:,6) p2(:,5)];
%     aPoints(1) = aPointsN(1);
    pn1 = [p1 p2(:,5:8)];
    pn2 = [p2 p2(:,6:8) p2(:,5)];
...
%     
%     P1N_w = [P1_w W2(:,5:8) W2(:,5:6) P1_w(:,1:4) P1_w(:,1:4)];
%     P2N_w = [W2 W2(:,6:8) W2(:,5) W2(:,7:8) W2(:,6) W2(:,5) W2(:,8) W2(:,7) W2(:,8) W2(:,7) W2(:,6) W2(:,5)];
%     
%     

    [R, T, err] = LPnL_Bar_ENull0(pn1(1:2,:), pn2(1:2,:), P1N_w, P2N_w);
    if err > 100
        err = errt;
%         R = tR;
        T = tT;
    end
%     aPoints = CalculateAbyR0(p1, p2, P1_w, P2_w, R);
...
%         d=P2_w(:,1:5)-P1_w(:,1:5);
%         d = xnorm(d);
%         rot_wc = Roptimzation(p1(1:2,1:5), p2(1:2,1:5), IA, rot_wc, d, 10);
