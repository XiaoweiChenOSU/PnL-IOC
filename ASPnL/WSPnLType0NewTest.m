function [R, T, err] = WSPnLType0NewTest(p1, p2, P1_w, P2_w)
...
for HowToChooseFixedTwoLines = 1:3
    ...
    for rootId = 1 : numOfRoots
        ...
        %have: scaleproduct(Vi^c, ni^c) = 0
        %where Vi^c = Rwc * Vi^w
        %Using the above constraint to construct linear equation system Mat about 
        %[costheta, sintheta, 1].
        Mat = zeros(n, 3);
        for i = 1:n
            ...
            Mat(i, 1) = nxi * Vxi + nzi * Vzi;
            Mat(i, 2) = nxi * Vzi - nzi * Vxi;
            Mat(i, 3) = nyi * Vyi;
        end
        %solve the linear system Mat * [costheta, sintheta, tx, ty, tz, 1]' = 0  using SVD,
        [UMat, SMat, VMat] = svd(Mat);
        vec = VMat(:,3);% the last column of Vmat;
        vec = vec/vec(3); %the condition that the last element of vec should be 1.
        ...
        R_truth = [[ 0.99923861  -0.01744177  -0.0348995 ]
                   [ -0.01865956  -0.99921736 -0.03487824]
                    [-0.03426384  0.03550289  -0.99878203]]

        
        errR1 = cal_rotation_err(rot_wc,R_truth);
        
        MatN = zeros(8, 4);
        for i = 1:n
            nxi = n_c(1,i);  nyi = n_c(2,i);  nzi = n_c(3,i);
            Pc = rot_wc * Pw(:,i);
            Pxi = Pc(1);        Pyi = Pc(2);        Pzi = Pc(3);
            % apply the constraint scaleproduct(Pi^c, ni^c) = 0
            MatN(i, 1) = nxi;
            MatN(i, 2) = nyi;
            MatN(i, 3) = nzi;
            MatN(i, 4) = nxi * Pxi + nyi * Pyi + nzi * Pzi;    
        end
        %solve the linear system Mat * [costheta, sintheta, tx, ty, tz, 1]' = 0  using SVD,
        [UTMat, STMat, VTMat] = svd(MatN);
        vecTN = VTMat(:,4);% the last column of Vmat;
        vecTN = vecTN/vecTN(4); %the condition that the last element of vec should be 1.      
     
        pos = vecTN(1:3);
        
        T_truth = [-3.634668738400000e+02  1.452080390400000e+02  5.282636233600000e+02]
       
        errT1 = cal_translation_err(pos,T_truth')
        ...
        for i = 1:4
            ...
            MatN(i, 6) = -nyi;
            MatN(i, 7) = -nzi;
            MatN(i, 8) = nxi * Pxi + nyi * Pyi + nzi * Pzi;    
        end 
        ...
        for i = 5:8
            ...
            MatN(i, 6) = -nyi;
            MatN(i, 7) = -nzi; 
            MatN(i, 8) = ((P1N_w(1,i)+P2N_w(1,i))*(nxi*Rw_c(1,1)+nyi*Rw_c(2,1)+nzi*Rw_c(3,1))/2)+ ((P1N_w(2,i)+P2N_w(2,i))*(nxi*Rw_c(1,2)+nyi*Rw_c(2,2)+nzi*Rw_c(3,2))/2) + (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
        end
        
        for i = 9:13          
            % apply the constraint scaleproduct(Vi^c, ni^c) = 0
            if i>9 %because that if i=1, scaleproduct(Vi^c, ni^c) always be 0
                nxi = nc_nbar(1,i-1);  nyi = nc_nbar(2,i-1);  nzi = nc_nbar(3,i-1);
                MatN(2*i-10, i-9) = nxi * Rw_c(1,3) + nyi * Rw_c(2,3) + nzi * Rw_c(3,3);
                if i-8 > 4
                    MatN(2*i-10, 1) = -(nxi * Rw_c(1,3) + nyi * Rw_c(2,3) + nzi * Rw_c(3,3));
                else
                    MatN(2*i-10, i-8) = -(nxi * Rw_c(1,3) + nyi * Rw_c(2,3) + nzi * Rw_c(3,3));
                end 
                MatN(2*i-10, 8) = (nxi * Rw_c(1,1) + nyi * Rw_c(2,1) + nzi * Rw_c(3,1))*(P1N_w(1,i-1)-P2N_w(1,i-1))+(nxi * Rw_c(1,2) + nyi * Rw_c(2,2) + nzi * Rw_c(3,2))*(P1N_w(2,i-1)-P2N_w(2,i-1));

            end
            % apply the constraint scaleproduct(Pi^c, ni^c) = 0
            if i < 13
                nxi = nc_nbar(1,i);  nyi = nc_nbar(2,i);  nzi = nc_nbar(3,i); 
                MatN(2*i-9, i-8) = (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
                if i-7 > 4
                    MatN(2*i-9, 1) = (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
                else
                    MatN(2*i-9, i-7) = (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
                end
                MatN(2*i-9, 5) = -nxi;
                MatN(2*i-9, 6) = -nyi;
                MatN(2*i-9, 7) = -nzi;
                MatN(2*i-9, 8) = ((P1N_w(1,i)+P2N_w(1,i))*(nxi*Rw_c(1,1)+nyi*Rw_c(2,1)+nzi*Rw_c(3,1))/2)+ ((P1N_w(2,i)+P2N_w(2,i))*(nxi*Rw_c(1,2)+nyi*Rw_c(2,2)+nzi*Rw_c(3,2))/2) + (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;    
            end
        end   
        ...
        P1N_w(3,n+1:nn) = vecN(1:4);
        P2N_w(3,5:n) = vecN(1:4);
        P2N_w(3,n+1:nn-1) = vecN(2:4);
        P2N_w(3,nn) = vecN(1);
        
        [Vw, Pw] = getVP(P1N_w, P2N_w);
...
% 		[rot_wc, pos_wc, err] =  GN(pn1, pn2, P1N_w, P2N_w, rot_wc);
        P2_w =  P2N_w(:,1:8);
...
errpro = minimalReprojectionError;
