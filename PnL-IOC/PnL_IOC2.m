function [R, T, err, aPointsN] = PnL_IOC2(p1, p2, P1_w, P2_w,IA)
...
%     [tR, tT, errt] = SRPnL1(p1(1:2,:), p2(1:2,:), P1_w, P2_w);  
    ...
    [tR, tT, errt] = SRPnL2(p1(1:2,:),p2(1:2,:),P1_w, P2_w);
%     if isinf(errt)
...
%     tR = Roptimzation(p1(1:2,:), p2(1:2,:), IA, tR, d, 20);
    ...
    aPoints = CalculateAbyR2(p1, p2, P1_w, P2_w, tR);
    
    
    p = [p1(:,1) p2];
    W = [P1_w(:,1) P2_w];
    [EE,GG,CC] = RefineEquA2(p, W);
    solutionO = [Cayley(tR);aPoints]; 
    [solution, obj_cur] = RefineGaussNewtonRandApT2(solutionO,EE,GG, CC, p1, p2, P1_w, P2_w);
    if size(solution) ~= 13
...
%     [R,T] = invertRT(R0,T0);
    ...
    [R, T, err] = SRPnL2(pn1(1:2,:),pn2(1:2,:),P1N_w, P2N_w);
%     if isinf(err)
%        [R,T,err] = LPnL_Bar_ENull2(pn1(1:2,:), pn2(1:2,:), P1N_w, P2N_w); 
%     end
