function [R, T, err] = HASPnL(p1, p2, P1_w, P2_w, R_truth)
...
%	 T: estimated translation
    ...
    [R, T, err] = RSPnL_(p1, p2, P1_w, P2_w,R_truth);  
%     [R,T] = invertRT(R0,T0);
...
function [rot_wc, pos_wc,errpro] = RSPnL_(p1,p2,P1_w,P2_w,R_truth)
...
for HowToChooseFixedTwoLines = 1:3
    ...
    for rootId = 1 : numOfRoots
        ...
        errR = cal_rotation_err(rot_wc,R_truth);
        
        if errR < minimalReprojectionError
            minimalReprojectionError = errR;
...
rot_wc = optimumrot_cw;
pos_wc = optimumpos_cw;
