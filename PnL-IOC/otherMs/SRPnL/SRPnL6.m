...
function [ R_wc,t_wc ,minErr] = SRPnL6( p1, p2, W1, W2, cH )
...
%         s1 = atan2(Rr(2,1),Rr(1,1));
%         s2 = atan2(-Rr(3,1),sqrt(Rr(3,2)^2+Rr(3,3)^2));
%         s3 = atan2(Rr(3,2),Rr(3,3));
%         if abs(cT(2) - cH) > 1 || s1 >0.05 || s1 <-0.05 || s2 >0.05 || s2 <-0.05 || s3  < 3.10 || s3  > -3.10
%             continue;
%         end
        curErr=sr.'*GG*sr;
        if curErr<minErr
