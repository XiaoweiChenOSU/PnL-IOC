...
function [ R_wc,t_wc ,minErr] = SRPnL71(p1,p2,W1,W2,cH,Lheight)
...
isi = sum(isinf(polyDF));
isn = sum(isnan(polyDF));
...
for rootId = 1 : numOfRoots
    ...
    for j=1:length(c)
        ...
        Rt = CalculateTbyRK(p1, p2, W1, W2, cH, Rr);

        
        cT = -inv(Rr)*Rt;
        if cT(1) < 0 || cT(2) < 0 || cT(3) < 0 || cT(1) > Lheight || cT(2) > Lheight || cT(3) > Lheight || Rt(2) < 0
            continue;
        end
...
%             [solutioncH, errcH] = RefineGaussNewtonCH(Cayley(RrA),EEcH,GGcH,CCcH,p1,p2,W1,W2);
%             s1=solutioncH(1);
%             s2=solutioncH(2);
%             s3=solutioncH(3);
%             factor=1/(1+s1^2+s2^2+s3^2); 
...
%                        2*s3+2*s1*s2,1-s1^2+s2^2-s3^2,2*s2*s3-2*s1;
%                        2*s1*s3-2*s2,2*s1+2*s2*s3,1-s1^2-s2^2+s3^2];              
%             Rrt = CalculateTbyRK(p1, p2, W1, W2, cH, RrA);         
%         end
