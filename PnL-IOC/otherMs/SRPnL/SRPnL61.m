...
function [ R_wc,t_wc ,minErr] = SRPnL61(p1,p2,W1,W2,C_truth,Lheight)
...
for rootId = 1 : numOfRoots
    ...
    for j=1:length(c)
        ...
        Rt = CalculateTbyRK(p1, p2, W1, W2, C_truth, Rr);

        
        cT = -inv(Rr)*Rt;
        if cT(1) < 0 || cT(2) < 0 || cT(3) < 0 || cT(1) > Lheight || cT(2) > Lheight || cT(3) > Lheight
            continue;
        end
