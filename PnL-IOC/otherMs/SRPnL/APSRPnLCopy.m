...
function [ R_wc,t_wc, Oaerrs] = APSRPnLCopy(p1,p2,W1,W2,IA,po1,cH,po2,R_truth,T_truth,WT2,pnn1)
% ,minErr 
% P1N_w = [W1 W2];
...
%        [R23, T23, errs23] = SRPnL(pn1(1:2,:), pn2(1:2,:), Pn1_w1, Pn2_w1);
        
        for i = 1:20
            [EEA,GGA,CCA] = refineEquation(p1,pn1,pn2,Pn1_w1,Pn2_w1,RrA,cH);
            s = [Cayley(RrA);aPoints;Rt];
%             s = [Cayley(RrA);WT2(1:1);WT2(2,2);WT2(3,3);T_truth];
...
%             s = [Cayley(RrA);100;100;100];
%             [solutionA, errA] = CopyRefineGaussNewtonForApoints(s,EEA,GGA,W1,W2,U,CCA,IA);
% 
% 
%             s1=solutionA(1);s2=solutionA(2);s3=solutionA(3);
%             s4 =solutionA(4);s5=solutionA(5); s6=solutionA(6); 
...
%         [RrA, RAt, curErr] = ASPnL(pn1(1:2,:), pn2(1:2,:), Pn1_w1, Pn2_w1);
...
% %         [Rerr,Urep] = reprojection_error_usingRT(Xw,U,Rr,Rt,IA);
        ...
        curErr2 = getErr(p1,p2,Pn1_w1,Pn2_w1,RrA,RAt);
        Gn1_w1 = [W1 WT2'];
        Gn2_w1 = [WT2; WT2(2:3,:); WT2(1,:)]';
        GErr = getErr(pn1,pn2,Pn1_w1,Pn2_w1,R_truth,T_truth);
        pns1 = [pnn1 p2];
        GcurErr = getErr(pns1,pn2,Gn1_w1,Gn2_w1,R_truth,T_truth);
        GcurErr1 = getErr(pnn1,p2,W1,W2,R_truth,T_truth);
%         curErr = getErr(p1,p2,Pn1_w1,Pn2_w1,Rr,Rt);
...
errPf = getFirstErr(p1,p2,W1,W2,R_wc);

% errPftest = getFirstErr(p1,p2,W1,W2,R_truth);


% errPf = getErr(pf1,pf2,PW1,PW2,R_wc,t_wc);
