...
function [ R_wc,t_wc,Oerrs, Rerrs] = WSRPnLf3ls(p1,p2,W1,W2,IA,po1,cH,po2,R_truth,T_truth)
% ,minErr 
% P1N_w = [W1 W2];
...
%         MatN(4,1) = f;
%         MatN(4,3) = u0-po1(1,1);
%         MatN(4,4) = (f*Rw_c(1,1) + u0*Rw_c(3,1) - po1(1,1)*Rw_c(3,1))*W1(1,1)+(f*Rw_c(1,2) + u0*Rw_c(3,2) - po1(1,1)*Rw_c(3,2))*W1(2,1) + (f*Rw_c(1,3) + u0*Rw_c(3,3) - po1(1,1)*Rw_c(3,3))*W1(3,1);
%         MatN(5,2) = f;
%         MatN(5,3) = v0-po1(1,2);
%         MatN(5,4) = (f*Rw_c(2,1) + v0*Rw_c(3,1) - po1(1,2)*Rw_c(3,1))*W1(1,1)+(f*Rw_c(2,2) + v0*Rw_c(3,2) - po1(1,2)*Rw_c(3,2))*W1(2,1) + (f*Rw_c(2,3) + v0*Rw_c(3,3) - po1(1,2)*Rw_c(3,3))*W1(3,1);
...
%         aPoints = vecN(1:3);
        ...
        cT = -inv(Rw_c)*Rt;
        if cT(1) < 0 || cT(2) < 0 || cT(3) < 0 || cT(1) > 256 || cT(2) > 256 || cT(3) > 256 || Rt(2) < 0
            continue;
        end
...
%         
%         [rot_wc, pos_wc, curErr] =  GN(pn1, pn2, Pn1_w, Pn2_w, Rw_c);
%         [rot_wc, pos_wc, curErr] =  GN(p1, p2, W1, W2, Rw_c);
...
%         aPointsCh = aPoints;
        for i = 1:5
            ...
            [EEcH,GGcH,CCcH] = refineEquationForCH(p1, p2,W1,W2,cH);
...
%         curErr = cal_translation_err(Rt,T_truth);
        curErr = getErr(p1,p2,W1,W2,Rr,Rt);
