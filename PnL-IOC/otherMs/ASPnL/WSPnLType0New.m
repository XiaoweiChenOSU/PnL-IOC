function [R, T, P3P,err] = WSPnLType0New(p1, p2, P1_w, P2_w)
...
%	 T: estimated translation
    ...
    [R0, T0, P3P, err] = RSPnL_(p1, p2, P1_w, P2_w);  
    [R,T] = invertRT(R0,T0);
...
function [rot_cw, pos_cw, P3P,errpro] = RSPnL_(p1,p2,P1_w,P2_w)
...
%         nc_nbar = Rot * n_nc;
        ...
        Rw_c = rot_wc;
        for i = 1:4
            nxi = nc_nbar(1,i+4);  nyi = nc_nbar(2,i+4);  nzi = nc_nbar(3,i+4);
            MatN(i, i) = (nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
            MatN(i, 5) = -(nxi*pos(1)+nyi*pos(2)+nzi*pos(3))+((P1N_w(1,i)+P2N_w(1,i))*(nxi*Rw_c(1,1)+nyi*Rw_c(2,1)+nzi*Rw_c(3,1))/2)+ ((P1N_w(2,i)+P2N_w(2,i))*(nxi*Rw_c(1,2)+nyi*Rw_c(2,2)+nzi*Rw_c(3,2))/2) + P1N_w(3,i)*(nxi*Rw_c(1,3)+nyi*Rw_c(2,3)+nzi*Rw_c(3,3))/2;
        end
        ...
        for i = 5:9          
            ...
            if i < 9
                ...
                MatN(2*i-5, 5) =  -(nxi*pos(1)+nyi*pos(2)+nzi*pos(3))+((P1N_w(1,j)+P2N_w(1,j))*(nxi*Rw_c(1,1)+nyi*Rw_c(2,1)+nzi*Rw_c(3,1))/2)+ ((P1N_w(2,j)+P2N_w(2,j))*(nxi*Rw_c(1,2)+nyi*Rw_c(2,2)+nzi*Rw_c(3,2))/2);    
            end
        end   
